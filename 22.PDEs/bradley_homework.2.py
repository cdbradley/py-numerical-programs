...
v = np.zeros(N+1)
vm = np.zeros(N+1) # "v_minus" = v at previous timestep
vp = np.zeros(N+1) # "v_plus"  = v at next timestep
...
while n < nts:
    for i in range(1,N,1):
        #first step
        ym[i] = y[i] +(dt/2)*v[i]
        vm[i] = v[i] +(dt/2)*c**2*((y[i+1]-2*y[i]+y[i-1])/dx**2)
        #second step
        yp[i] = y[i]+dt* v[i]
        vp[i] = v[i] +(dt/2)*c**2*((ym[i+1]-2*ym[i]+ym[i-1])/dx**2)
